...
execute as @e[type=marker,tag=current,tag=y_sort] run data modify entity @s Pos[1] set from storage txd:storage root.array.y_sort.pos_temp
execute as @e[type=marker,tag=current,tag=y_sort] run data modify entity @s data.y_sort_data set from storage txd:storage root.array.y_sort.input[0]
