...
execute store result score $candidate_min TXDGlobal run data get storage txd:storage root.list.selection_sort.candidateMin

# Set new min to first element of list, and put the old min in place of the new mins original place
execute if score $curr_min TXDGlobal > $candidate_min TXDGlobal run data modify storage txd:storage root.list.selection_sort.input[0] set from storage txd:storage root.list.selection_sort.currMin
execute if score $curr_min TXDGlobal > $candidate_min TXDGlobal run data modify storage txd:storage root.list.selection_sort.currMin set from storage txd:storage root.list.selection_sort.candidateMin
